...
h = tf.kera.layers.Activation('swish')(h)
...
y = tf.keras.layers.Dense(3, activation='softmax')(h) # (확률)분류 예측, Softmax, Sigmoid
model = tf.keras.models.Model(x, y)
...
# train model
model.fit(independent, dependent, epochs = 1000, batch_size = 150, verbose=0)
